...
export class FacetBuilder {
   ...
   */
  count(maxFacetTerms: number): FacetBuilder {
   ...
   */
  sortByCount(dir: 'asc' | 'desc' = 'desc'): FacetBuilder {
   ...
   * @param dir sort direction
   */
  sortByValue(dir: 'asc' | 'desc' = 'asc'): FacetBuilder {
   ...
   */
  values(...values: Array<number | Date>): FacetBuilder {
   ...
   */
  interval(value: number | FacetDateInterval): FacetBuilder {
   ...
   */
  timeoffset(hours: number, minutes = 0): FacetBuilder {
    const sign = hours < 0 ? '-' : '';
